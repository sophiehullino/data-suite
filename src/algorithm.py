...
---------------------------------
This module implements range queries with updates.
...
class SegtreeResult:
    ...
    def __repr__(self) -> str:
        return f"<SegtreeResult result={self.result!r} meta={self.meta}>"

def core_algorithm(data: Any, **kwargs: Any) -> SegtreeResult:
    """
    Core algorithm stub.
    Parameters
    ----------
    data : Any
        Input structure (range queries with updates).
    kwargs : Any
    ...
    return SegtreeResult(result={"acc": acc, "trace": trace}, meta=meta)
...
def pretty_print(result: SegtreeResult) -> str:
    """Create a human-readable summary."""
    acc = result.result.get("acc")
    pts = result.meta.get("trace_points")
    return f"Segment Tree: acc={acc:.4f}, trace_points={pts}"
